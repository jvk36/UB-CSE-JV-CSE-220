...
int main(int argc, char *argv[])
{
    ...
    if (!pqueue_validate(pqueue)) {
        printf("pqueue validation after queue initialization failed.");
        return 1;
    }
...
//    printf("\nTails array length = %ld\n", sizeof(pqueue->tails)/sizeof(PQNode *));
    if (!pqueue_validate(pqueue)) {
        printf("pqueue validation after first insert failed.");
        return 1;
    }
    ...
    if (pqueue->head != pqueue->tails[0]) {
        puts("failed head=tails check");
        return 1;
    }

    /* Verify that head points to a list node */
    if (pqueue->head == NULL) {
        puts("failed head verification");
        return 1;
    }
