...
void pcq_destroy(PCQueue pcq) {
 ...
 // if empty queue, setup to release the threads waiting on pcq_retrieve.
    while (pcq->front == pcq->back) {
        usleep(1000);
        pcq_insert(pcq, NULL);
    }

 // if queue not empty, setup to release any threads waiting on pcq_insert.    
    while (pcq->front != pcq->back) {
        usleep(1000);
        pcq_retrieve(pcq);
    }
